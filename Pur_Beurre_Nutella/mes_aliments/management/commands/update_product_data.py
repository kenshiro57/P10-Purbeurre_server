    ...
    def handle(self, *args, **options):
        ...
        for product in Product.objects.all():
            ...
            for i in range(len(self.json_data['products'])):
                ...
                if product.url != None and self.product_data.get(
                        'url') == product.url:
                    if self.product_data.get('name') != None:
                        product.name = self.product_data.get('name')
                    if self.product_data.get('brands') != None:
                        product.brands = self.product_data.get('brands')
                    if self.product_data.get('nutriscore_grade') != None:
                        product.nutriscore_grade = self.product_data.get(
                            'nutriscore_grade')
                    if self.product_data.get('image_url') != None:
                        product.image = self.product_data.get('image_url')
                    if self.product_data.get('stores') != None:
